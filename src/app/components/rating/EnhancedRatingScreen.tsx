...
const generateRatingData = (): Student[] => {
  const classes = ['5-1', '5-2', '5-3', '6-1', '6-2', '6-3', '7-1', '7-2', '7-3', '8-1', '8-2', '8-3', '9-2', '10-3', '11-2', '11-3'];
  const names = [
    ['Алексеев', 'Павел', 'Сергеевич'],
    ['Борисова', 'Ксения', 'Андреевна'],
    ['Григорьев', 'Михаил', 'Иванович'],
    ['Дмитриева', 'Анастасия', 'Олеговна'],
    ['Евдокимов', 'Артём', 'Николаевич'],
    ['Жукова', 'Виктория', 'Петровна'],
    ['Захаров', 'Илья', 'Дмитриевич'],
    ['Зимина', 'Полина', 'Владимировна'],
    ['Киселёв', 'Роман', 'Евгеньевич'],
    ['Ларионова', 'Валерия', 'Максимовна'],
  ];
  ...
  classes.forEach((cls) => {
    ...
    for (let i = 0; i < studentsPerClass; i++) {
      ...
      const academic = Math.floor(Math.random() * 120) + 30;
      const extracurricular = Math.floor(Math.random() * 60) + 10;
      const project = Math.floor(Math.random() * 40) + 5;
      students.push({
        id: globalId++,
        lastName: name[0],
        firstName: name[1],
...
const GENERATED_RATING_DATA = generateRatingData();
...
export function EnhancedRatingScreen() {
  ...
  // Build real student rating from AppContext achievements
  const realRatingData: Student[] = useMemo(() => {
    return students.map((s) => {
      const stuAch = achievements.filter(a => a.studentId === s.id && a.status === 'approved');
      const academic = stuAch.filter(a => a.category === 'Учебные достижения').reduce((sum, a) => sum + a.points, 0);
      const extracurricular = stuAch.filter(a => a.category === 'Внеурочная деятельность').reduce((sum, a) => sum + a.points, 0);
      const project = stuAch.filter(a => a.category === 'Проектная деятельность').reduce((sum, a) => sum + a.points, 0);
      return {
        id: s.id,
        lastName: s.lastName,
  ...
  // Merge real + generated data
  const RATING_DATA: Student[] = useMemo(() => {
    return [...realRatingData, ...GENERATED_RATING_DATA];
  }, [realRatingData]);
