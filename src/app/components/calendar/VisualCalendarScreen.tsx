...
export function VisualCalendarScreen({ userRole }: { userRole?: 'admin' | 'curator' | 'student' }) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between"><div><h2 className="text-2xl text-gray-900 mb-2">Календарь событий</h2><p className="text-gray-600">Backend-driven календарь системы</p></div>{canAddEvents && <Button onClick={() => setIsAddDialogOpen(true)} className="bg-blue-600 hover:bg-blue-700 gap-2"><Plus className="w-4 h-4" />Добавить событие</Button>}</div>
