...
export function NewsAndAnnouncementsScreen({ canEdit }: { canEdit: boolean }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">{news.length === 0 ? <Card><CardContent className="p-6 text-center text-gray-500">Новостей пока нет</CardContent></Card> : news.map((item) => <Card key={item.id}><CardHeader><div className="flex justify-between items-start gap-2"><div><CardTitle className="text-lg flex items-center gap-2"><Newspaper className="w-4 h-4 text-blue-600" />{item.title}</CardTitle><p className="text-xs text-gray-500 mt-1">{item.date} • {item.author || 'Система'}</p></div><div className="flex gap-2 items-center"><Badge>{priorityLabel(item.priority)}</Badge>{canEdit && <Button variant="ghost" size="sm" onClick={() => deleteNewsItem(item.id)}><Trash2 className="w-4 h-4 text-red-600" /></Button>}</div></div></CardHeader><CardContent><p className="text-sm text-gray-700 whitespace-pre-wrap">{item.content}</p></CardContent></Card>)}</div>

      <Dialog open={isOpen} onOpenChange={setIsOpen}><DialogContent><DialogHeader><DialogTitle>Новая новость</DialogTitle></DialogHeader><div className="space-y-3"><Label>Заголовок</Label><Input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} /><Label>Содержимое</Label><Textarea value={form.content} onChange={(e) => setForm({ ...form, content: e.target.value })} rows={5} /><Label>Автор</Label><Input value={form.author} onChange={(e) => setForm({ ...form, author: e.target.value })} /><Label>Категория</Label><Select value={form.priority} onValueChange={(v: 'low' | 'normal' | 'high') => setForm({ ...form, priority: v })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="normal">Обычная новость</SelectItem><SelectItem value="high">Важная новость</SelectItem><SelectItem value="low">Низкий приоритет</SelectItem></SelectContent></Select><Button onClick={handleAdd} className="w-full">Опубликовать</Button></div></DialogContent></Dialog>
    </div>
  );
}
