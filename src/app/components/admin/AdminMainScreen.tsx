...
export function AdminMainScreen({ onNavigate }: AdminMainScreenProps) {
  const { users, students, achievements, auditLog } = useApp();
  ...
  const stats = [
    ...
    { label: 'Учеников в системе', value: String(students.length), icon: SettingsIcon, color: 'bg-orange-50 text-orange-600' },
  ];

  const recentActivity = auditLog.slice(0, 5).map(l => ({
    id: l.id,
    action: l.details,
    time: l.timestamp.split(' ')[1] ?? l.timestamp,
    type: l.entity.toLowerCase(),
  }));
