...
export function CuratorRequestsScreen() {
  ...
  const parseDocument = (raw: string) => {
    if (!raw.startsWith('upload::')) return { name: raw, url: '' };
    const [, fileName = 'document', dataUrl = ''] = raw.split('::');
    return { name: fileName, url: dataUrl };
  };
